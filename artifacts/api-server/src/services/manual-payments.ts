...
export function toEnglishDigits(value: string): string {
  return value.replace(/[٠-٩۰-۹]/g, (digit) => {
    ...
    return String(
      arabicIndex >= 0 ? arabicIndex : persianDigits.indexOf(digit),
    );
  });
}
...
export function calculateRequiredPayment(
  ...
  const confirmationAmount = safeTotal > 2000 ? 150 : 100;
  return plan === "full" ? safeTotal : Math.min(confirmationAmount, safeTotal);
}
...
export function assessPaymentRisk(input: {
  ...
  if (input.hasRejectedSenderAttempt)
    reasons.push("sender_previously_rejected");
  if (input.hasDuplicateTransactionReference)
    reasons.push("transaction_reference_duplicate");
  if (input.hasDuplicateProof) reasons.push("proof_duplicate");
  if (
    input.hasRejectedSenderAttempt ||
    input.hasDuplicateTransactionReference ||
    input.hasDuplicateProof
  )
...
export async function reviewPaymentAttempt(input: {
  ...
  return db.transaction(async (tx) => {
    await tx.execute(
      sql`select id from payment_attempts where id = ${input.attemptId} for update`,
    );
    const [attempt] = await tx
      .select()
      .from(paymentAttemptsTable)
      .where(eq(paymentAttemptsTable.id, input.attemptId));
    if (!attempt)
      throw new PaymentReviewError("NOT_FOUND", "بيانات التحويل غير موجودة");
    ...
    if (!order)
      throw new PaymentReviewError(
        "NOT_FOUND",
        "الطلب المرتبط بالتحويل غير موجود",
      );
    const [employee] = await tx
      .select({ name: usersTable.name })
      .from(usersTable)
      .where(eq(usersTable.id, input.employeeId));
    ...
    if (attempt.status === "rejected" || attempt.status === "confirmed") {
      throw new PaymentReviewError(
        "INVALID_STATE",
        "تم اتخاذ قرار نهائي على هذا التحويل بالفعل",
      );
    }
    ...
    if (
      ...
      attempt.transactionReferenceNormalized
    ) {
      ...
      await tx.execute(
        sql`select pg_advisory_xact_lock(hashtextextended(${attempt.transactionReferenceNormalized}, 0))`,
      );
      const [duplicate] = await tx
        ...
        .where(
          and(
            eq(
              paymentAttemptsTable.transactionReferenceNormalized,
              attempt.transactionReferenceNormalized,
            ),
      ...
      if (duplicate && !input.overrideDuplicateReference) {
        throw new PaymentReviewError(
          "DUPLICATE_REFERENCE",
          "رقم العملية مؤكد بالفعل على طلب آخر. يلزم Override إداري مسجل.",
        );
      }
      if (
        duplicate &&
        input.overrideDuplicateReference &&
        !input.notes?.trim()
      ) {
        throw new PaymentReviewError(
          "OVERRIDE_REASON_REQUIRED",
          "اكتب سبب التجاوز الإداري لرقم العملية المكرر",
        );
      }
    }
    ...
    if (input.decision === "confirmed") {
      const [confirmed] = await tx
        .select({
          total: sql<string>`coalesce(sum(${paymentAttemptsTable.amount}::numeric), 0)::text`,
        })
        .from(paymentAttemptsTable)
        .where(
          and(
      ...
      paymentStatus =
        input.decision === "rejected" ? "rejected" : "needs_review";
    }
